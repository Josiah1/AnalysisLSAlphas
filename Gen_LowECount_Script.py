...
if __name__ == "__main__":
    ...
    StreamListDir = "/dybfs2/users/lijj16/DATA/Retw/IBD/H_U235/Output/" + site

    basedir = "./counting_result_lowE/"
    ...
    for runIdx, run in enumerate(sorted(ListOfList)):
        ...
        if ext == ".root":
            ...
            if os.path.exists(outputfilename) and os.path.exists(outputinfoname):
                continue
            ...
            FILE.writelines(
                f"./AnalysisLSAlphas_lowE {inputfilename} {outputfilename} {outputinfoname}"
            )
            FILE.close()
    ...
    os.system(
        "hep_sub -g dyw " + parent_script + " -argu %{ProcId} -n " + str(job_count)
    )
    """
